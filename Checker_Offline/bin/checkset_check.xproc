...
======================================================================== 
-->
<p:declare-step type="bc:check" name="Check" 
    ...
    <p:try name="ChecksetPrefil">
        <p:group>
            ...
            <p:xslt name="Checkset_Prefill">
                ...
                <p:input port="stylesheet">
                    <p:document href="checkset_generate_check.xsl"/>
                </p:input>
            </p:xslt>
            <p:choose>
                <p:when test="$debug != 0">
                    <p:store>
                        <p:with-option name="href" select="concat('../work/checkset_check_', $type, '.xsl')"/>
                    </p:store>
                </p:when>
                <p:otherwise>
            ...
            <p:xslt name="Checkset_Check">
                <p:input port="source">
                    <p:pipe port="document" step="Check"/>
                </p:input>
                <p:input port="stylesheet">
        ...
        <p:catch name="catch">
            ...
            <p:output port="result" primary="true">
                <p:inline>
                    <er:error>Checkset Check - the XPath checks are wrong.</er:error>
                </p:inline>
            </p:output>
            <p:output port="report">
    ...
    <p:pack wrapper="result" name="ChecksetResult">
        <p:input port="source">
            <p:pipe port="result" step="ChecksetPrefil"/>
        </p:input>
        <p:input port="alternate">
            <p:pipe port="report" step="ChecksetPrefil"/>            
        </p:input>
    </p:pack>
    <p:choose>
        <p:when test="$debug != 0">
            <p:store>
                <p:with-option name="href" select="concat('../work/checkset_check_', $type, '.xml')"/>
            </p:store>
        </p:when>
        <p:otherwise>
            <p:sink name="discarding-debugging-output"/>
    ...
    <p:identity>
        <p:input port="source">
            <p:pipe port="result" step="ChecksetResult"/>
        </p:input>
    </p:identity>
    <p:unwrap match="result/result"/>
