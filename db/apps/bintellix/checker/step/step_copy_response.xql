...
let $step_new := request:get-parameter('step_new','StepNew')
...
if (not(xmldb:collection-available($data-collection-source))) then
    ...
    <error>Source step '{$step}' does not exist.</error>
	...
	if (doc(concat($data-collection-destination,$filename-destination))/cs:step/cs:identity/cs:step-id != $step_new) then
	   <error>Copy of group '{$group}' step '{$step}' to group '{$group_new}' step '{$step_new}' failed</error>
	else
