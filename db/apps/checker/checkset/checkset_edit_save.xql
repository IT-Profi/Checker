...
let $step-file := concat("step_", $step, ".xml")
let $store := xmldb:store(concat($ct-home-data,'/', $group), $step-file, $item)
(:let $xxx := xmldb:update($checkset-location, $xupdate):)

return 
<html xmlns="http://www.w3.org/1999/xhtml">
    <body>
        <subtitle>Step {$group}/{$step} (a checkset) updated successfully.</subtitle>
    </body>
</html>