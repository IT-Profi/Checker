...
(: RULESET handling :)	
    ...
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/ruleset/ruleset_{$params[1]}.xql">
            <set-attribute name="ct-home-web" value="{$ct-home-web}"/>
            <set-attribute name="ct-home-data" value="{$ct-home-data}"/>
            <add-parameter name="type" value="GLOBAL"/>
        </forward>        
        <view>
            <forward servlet="XSLTServlet">
                <set-attribute name="xslt.stylesheet" value="{concat($exist:root, $exist:controller, '/frame.xsl')}"/>
    ...
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/ruleset/ruleset_{$params[2]}.xql">
            <set-attribute name="ct-home-web" value="{$ct-home-web}"/>
            <set-attribute name="ct-home-data" value="{$ct-home-data}"/>
            <add-parameter name="group" value="{$params[1]}"/>
            <add-parameter name="type" value="GROUP"/>
        </forward>
        <view>
            <forward servlet="XSLTServlet">
                <set-attribute name="xslt.stylesheet" value="{concat($exist:root, $exist:controller, '/frame.xsl')}"/>
...
(: GROUP handling :)
else if (matches($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/(copy|copy_response|create|create_response|delete|delete_response|edit|edit_response|overview)\.html$')) then
	let $params := subsequence(text:groups($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/(copy|copy_response|create|create_response|delete|delete_response|edit|edit_response|overview)\.html$'), 2)
	return
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/group/group_{$params[2]}.xql">
            <set-attribute name="ct-home-web" value="{$ct-home-web}"/>
            <set-attribute name="ct-home-data" value="{$ct-home-data}"/>
            <add-parameter name="group" value="{$params[1]}"/>
        </forward>
...
(: STEP handling :)
...
else if (matches($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/step/([a-zA-Z][a-zA-Z0-9_-]+)/(create|create_response|copy|copy_response|delete|delete_response|move|order|services)\.html$')) then
	let $params := subsequence(text:groups($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/step/([a-zA-Z][a-zA-Z0-9_-]+)/(create|create_response|copy|copy_response|delete|delete_response|move|order|services)\.html$'), 2)
	return
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/step/step_{$params[3]}.xql">
            ...
            <set-attribute name="ct-home-data" value="{$ct-home-data}"/>
            <add-parameter name="group" value="{$params[1]}"/>
            <add-parameter name="step" value="{$params[2]}"/>
        </forward>
        <view>
            <forward servlet="XSLTServlet">
                <set-attribute name="xslt.stylesheet" value="{concat($exist:root, $exist:controller, '/frame.xsl')}"/>
...
else if (matches($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/step/([a-zA-Z][a-zA-Z0-9_-]+)/result(.*)\.xml$')) then
	let $params := subsequence(text:groups($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/step/([a-zA-Z][a-zA-Z0-9_-]+)/result(.*)\.xml$'), 2)
	return
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
else if (matches($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/step/([a-zA-Z][a-zA-Z0-9_-]+)/result(.*)\.html$')) then
	let $params := subsequence(text:groups($exist:path, '^/([a-zA-Z][a-zA-Z0-9_-]+)/step/([a-zA-Z][a-zA-Z0-9_-]+)/result(.*)\.html'), 2)
	return
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-data,'/db/apps')}/{$params[1]}/result/step_{$params[2]}.xml">
            <set-attribute name="ct-home-data" value="{$ct-home-data}"/>
	...
	return
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/step/step_run.xql">
            ...
            <add-parameter name="group" value="{$params[1]}"/>
            <add-parameter name="step" value="{$params[2]}"/>
            <add-parameter name="format" value="{if ($params[3]='xml') then 'xml' else 'report'}"/>
        </forward>
        { if ($params[3]!='xml') then
            <view>
                <forward servlet="XSLTServlet">
                    <set-attribute name="xslt.stylesheet" value="{concat($exist:root, $exist:controller, '/checkset/checkset_result_report.xsl')}"/>
                    <set-attribute name="xslt.group" value="{$params[1]}"/>
                ...
                <forward servlet="XSLTServlet">
                    ...
                    <set-attribute name="xslt.group" value="{$params[1]}"/>
                    <set-attribute name="xslt.step" value="{$params[2]}"/>
                    <set-attribute name="xslt.filename" value="run.{$params[3]}"/>
                    <!--
	...
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/step/step_download.xql">
            ...
            <add-parameter name="group" value="{$params[1]}"/>
            <add-parameter name="step" value="{$params[2]}"/>
            <add-parameter name="filename" value="{$params[3]}.xml"/>
        </forward>
	</dispatch>
...
(: RUNSET Handling :)
    ...
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{substring-after($ct-home-db,'/db/apps')}/runset/runset_download.xql">
            ...
            <set-attribute name="ct-home-data" value="{$ct-home-data}"/>
            <add-parameter name="group" value="{$params[1]}"/>
            <add-parameter name="runset" value="{$params[2]}"/>
            <set-attribute name="filename-pattern" value="{$config/bc:config/bc:set/bc:filename-pattern/text()}"/>            
        </forward>
    </dispatch>
